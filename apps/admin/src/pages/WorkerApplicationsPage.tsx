...
interface WorkerApplicationDTO {
  id: number;
  email: string;
  fullName: string;
  phone: string | null;
...
interface ApproveResponse {
  application: WorkerApplicationDTO;
  tempPassword: string;
}
...
export function WorkerApplicationsPage() {
  ...
  const [items, setItems] = useState<WorkerApplicationDTO[]>([]);
  const [loading, setLoading] = useState(true);
  const [merchants, setMerchants] = useState<MerchantSummaryDTO[]>([]);
  const [selected, setSelected] = useState<WorkerApplicationDTO | null>(null);
  ...
  useEffect(() => {
    api<MerchantSummaryDTO[]>('/admin/applications/merchants/list')
      .then(setMerchants)
      .catch(() => {});
  }, [api]);
  ...
  useEffect(() => {
    if (openId) {
      api<WorkerApplicationDTO>(`/admin/applications/workers/${openId}`)
        .then((data) => {
          setSelected(data);
  ...
  const refreshList = () => {
    ...
    q.set('page', '0');
    q.set('size', '100');
    api<{ content: WorkerApplicationDTO[] }>(`/admin/applications/workers?${q.toString()}`).then((data) => setItems(data.content));
  };

  const handleApprove = async () => {
    if (!selected || !approveMerchantId) {
      addToast('error', 'Select a merchant to assign the worker to');
      return;
    }
    setActionLoading(true);
    try {
      const res = await api<ApproveResponse>(`/admin/applications/workers/${selected.id}/approve`, {
        method: 'POST',
        body: JSON.stringify({ merchantId: approveMerchantId, adminNote: adminNote || undefined }),
      });
  ...
  const handleReject = async (note: string) => {
    ...
    setActionLoading(true);
    try {
      const updated = await api<WorkerApplicationDTO>(`/admin/applications/workers/${selected.id}/reject`, {
        method: 'POST',
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-neutral-900 mb-6">Worker Applications</h1>
      <div className="mb-4">
        <select
          value={statusParam}
          ...
          <table className="w-full text-sm">
            <thead className="bg-neutral-50 border-b border-neutral-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-neutral-700">Full name</th>
            ...
            <tbody>
              {items.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-neutral-900">{row.fullName}</td>
      ...
      <Drawer open={!!openId} onOpenChange={(o) => !o && closeDrawer()} title="Worker Application" side="right">
        {selected && (
          <div className="space-y-4 text-sm">
            ...
            <div><span className="text-neutral-500">Full name</span><br />{selected.fullName}</div>
            ...
            {selected.city && <div><span className="text-neutral-500">City</span><br />{selected.city}</div>}
            ...
            {tempPassword && (
              <div className="rounded border border-amber-200 bg-amber-50 p-3">
                ...
                <p className="text-xs text-neutral-600 mt-2">Account: {selected.email} · Role: WORKER</p>
              </div>
            )}
            ...
            {selected.status === 'PENDING' && (
              <>
                ...
                <div>
                  <label className="block text-neutral-500 mb-1">Assign to merchant (required for approve)</label>
                  <select
                    value={approveMerchantId}
                    onChange={(e) => setApproveMerchantId(e.target.value)}
                    className="w-full rounded border border-neutral-300 px-2 py-1 text-sm"
                  >
                ...
                <div className="flex gap-2">
                  <Button onClick={() => setApproveOpen(true)} disabled={!approveMerchantId}>Approve</Button>
      ...
      <ConfirmDialog
        ...
        description="This will create a worker account and assign them to the selected merchant. A temporary password will be shown once."
