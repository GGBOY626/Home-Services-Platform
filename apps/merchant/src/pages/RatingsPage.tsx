...
export function RatingsPage() {
  ...
  const load = () => {
    ...
    const base = qs ? `?${qs}` : '';
    Promise.all([
      api<RatingSummaryDTO>(`/merchant/ratings/summary${base}`),
      api<{ content: RatingDTO[] }>(`/merchant/ratings${base ? base + '&' : '?'}page=0&size=50`),
    ])
      .then(([s, p]) => {
        setSummary(s);
      ...
      {!loading && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <KpiCard title="Average rating" value={total > 0 ? avg.toFixed(1) : '—'} />
            <KpiCard title="Total ratings" value={String(total)} />
          </div>
          ...
          <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
            <p className="text-sm font-medium text-neutral-700 p-4 border-b border-neutral-200">Recent ratings</p>
            {ratings.length === 0 ? (
              <p className="p-6 text-neutral-500 text-sm">No ratings in this period.</p>
            ) : (
